...
app.get("/administrator", userMiddleware.isLoggedIn, async (req, res) => {
  res.render(__dirname + "/public/administrator/index.html", {
  ...
  if (req.body.sendkey) {
    if (req.body.sendkey == "asdn9n34b374b8734vasdv7v73v324") {
      ...
      Object.keys(data).forEach(async (key) => {
        if (key != "") {
          var result = await database.awaitQuery(
            `SELECT * FROM setting_data WHERE setting_name = ?`,
            [key]
          );

          if (result.length != 0) {
            await database.awaitQuery(
              `UPDATE setting_data SET setting_value = ? WHERE setting_name = ?`,
              [data[key], key]
            );
          } else {
            await database.awaitQuery(
              `INSERT INTO setting_data (setting_name, setting_value) VALUES (?, ?)`,
              [key, data[key]]
            );
          }
        }
...
app.post("/geterrors", async (req, res) => {
  if (req.body.sendkey) {
    if (req.body.sendkey == "asdn9n34b374b8734vasdv7v73v324") {
      ...
      var total_records = await database.awaitQuery(
        `SELECT COUNT(*) AS Total FROM error_log`
      );
      total_records = total_records[0].Total;

      var total_records_with_filter = await database.awaitQuery(
        `SELECT COUNT(*) AS Total FROM error_log WHERE 1 ${search_query}`
      );
      ...
      data_records.forEach(function (row) {
        ...
        data_arr.push({
          ...
          action:
            ...
            row.id +
            '" type="button" class="btn btn-link btn-sm btn-rounded">Löschen</button>',
        });
      });
  ...
  if (req.body.sendkey) {
    if (req.body.sendkey == "asdn9n34b374b8734vasdv7v73v324") {
      ...
      for (var i = 0; i < ids.length; i++) {
        await database.awaitQuery(`DELETE FROM error_log WHERE id = ?`, [
          ids[i],
        ]);
      }
...
app.post("/login", async (req, res) => {
  ...
  var row = await database.awaitQuery(
  ...
  if (row.length != 0) {
    // Check Password
    var passwordCheck = await bcrypt.compare(
      req.body.password,
      row[0].password
    );
  ...
  if (req.query.code) {
    ...
    axios({
      ...
      .catch(function (error) {
        errorlogging.saveError(
          "error",
          "hubspot",
          "Error register hubspot app",
          error
        );
...
app.post("/hubspotwebhook", async (req, res) => {
  const hubspotClient = new hubspot.Client({
    accessToken: await settings.getSettingData("hubspotaccesstoken"),
  });
  ...
  if (req.headers["x-hubspot-signature"]) {
    ...
    source_string =
      (await settings.getSettingData("hubspotclientsecret")) +
    ...
    if (gen_hash == req.headers["x-hubspot-signature"]) {
      ...
      if (body.subscriptionType) {
        ...
        if (
          ...
          body.propertyName == "dealstage" &&
          body.propertyValue == "contractsent"
        ) {
          ...
          try {
            ...
            var dealData = await hubspotClient.crm.deals.basicApi.getById(
              dealId,
              properties,
              undefined,
              associations,
            ...
            if (
              dealData.properties["docusign_contract_status"] == null ||
              dealData.properties["docusign_contract_status"] == "" ||
              dealData.properties["docusign_contract_status"] ==
                "nicht gesendet"
            ) {
              // Load Contact Data
              ...
              try {
                ...
                for (var i = 0; i < requireFields.length; i++) {
                  if (
                    contactData.properties[requireFields[i]] == null ||
                    contactData.properties[requireFields[i]] == ""
                  ) {
                    checkField = false;
                ...
                if (!checkField) {
                  ...
                  if (
                    contactData.properties.unternehmensform ==
                    "einzelunternehmen"
                  ) {
                  ...
                  if (dealData.properties["deal_gesellschaft"] == "DIS") {
                    var contractTemplateId = await settings.getSettingData(
                      "docusigndiscontracttemplateId"
                    );

                    var mailSubject = replacePlaceholder(
                      await settings.getSettingData("docusigndismailsubject"),
                      contactData.properties
                    );

                    var mailBody = replacePlaceholder(
                      await settings.getSettingData("docusigndismailbody"),
                      contactData.properties
                    );
                  } else if (
                    dealData.properties["deal_gesellschaft"] == "DIA"
                  ) {
                    var contractTemplateId = await settings.getSettingData(
                      "docusigndiacontracttemplateId"
                    );

                    var mailSubject = replacePlaceholder(
                      await settings.getSettingData("docusigndiamailsubject"),
                      contactData.properties
                    );

                    var mailBody = replacePlaceholder(
                      await settings.getSettingData("docusigndiamailbody"),
                      contactData.properties
                    );
                  }
                  ...
                  // Sent Contract
                  var row = await database.awaitQuery(
                    `SELECT * FROM access_token WHERE type = "docusign"`
                  );
                  docuSignApiClient.addDefaultHeader(
                    "Authorization",
                    "Bearer " + row[0].access_token
                  );

                  try {
                    var userInfo = await docuSignApiClient.getUserInfo(
                      row[0].access_token
                    );
                    var accountId = userInfo.accounts[0].accountId;
                    docuSignApiClient.setBasePath(
                      userInfo.accounts[0].baseUri + "/restapi"
                    );
                    docusign.Configuration.default.setDefaultApiClient(
                      docuSignApiClient
                    );

                    console.log(
                      date + " - done retrieving account info for user."
                    );
                    ...
                    envelopesApi.createEnvelope(
                      ...
                      async function (err, envelopeSummary, response) {
                        if (err) {
                          ...
                          var properties = {
                            docusign_contract_status: "nicht gesendet",
                            docusign_contract_status_fehler:
                              "Vertrag konnte nicht gesendet werden",
                          };
                          var SimplePublicObjectInput = { properties };
                          await hubspotClient.crm.deals.basicApi.update(
                            dealId,
                          ...
                          errorlogging.saveError(
                            "success",
                            "docusign",
                            "Vertrag erfolgreich gesendet (" +
                              envelopeSummary.envelopeId +
                              ")",
                            JSON.stringify(envelopeSummary)
                          );
                          console.log(
                            date +
                              " - EnvelopeSummary: " +
                ...
                errorlogging.saveError(
                  "error",
                  "hubspot",
                  "Error to load the Contact Data (" + contactId + ")",
                  ""
                );
                console.log(date + " - " + err);
              }
            ...
            errorlogging.saveError(
              "error",
              "hubspot",
              "Error to load the Deal Data (" + dealId + ")",
              ""
            );
            console.log(date + " - " + err);
          }
        ...
        if (
          ...
          body.propertyName == "export_software" &&
          body.propertyValue == "Fehler"
        ) {
          ...
          try {
            var contactData = await hubspotClient.crm.contacts.basicApi.getById(
              contactId,
              properties,
              undefined,
              associations,
            ...
            if (contactData.associations.deals.results.length != 0) {
              for (
                var i = contactData.associations.deals.results.length - 1;
                i >= 0;
                i--
              ) {
                ...
                if (dealData.properties.dealstage == "closedwon") {
                  if (
                    dealData.properties.deal_envelopeid != "" ||
                    dealData.properties.deal_envelopeid != null
                  ) {
            ...
            if (envelopeId != "") {
              ...
              try {
                ...
                var results = await envelopesApi.getFormData(
                  accountId,
                  envelopeId
                );
                ...
                for (var i = 0; i < docuSignFormData.length; i++) {
                  docuSignFormatData[docuSignFormData[i]["name"]] =
                    docuSignFormData[i]["value"];
                }
                var properties = {};
                var changeValue = false;

                if (
                  contactData.properties.bankname == "" ||
                  contactData.properties.bankname == null
                ) {
                ...
                if (
                  contactData.properties.bic == "" ||
                  contactData.properties.bic == null
                ) {
                ...
                if (
                  contactData.properties.iban == "" ||
                  contactData.properties.iban == null
                ) {
                ...
                if (
                  contactData.properties.kontoinhaber == "" ||
                  contactData.properties.kontoinhaber == null
                ) {
                ...
                if (
                  contactData.properties.mwst == "" ||
                  contactData.properties.mwst == null
                ) {
                ...
                if (Object.keys(properties).length != 0) {
                  ...
                  console.log(
                    date +
                      " - Success: Correct contact export error - " +
                      contactData.properties.email
                  );
                  errorlogging.saveError(
                    "success",
...
app.get("/successdocusignapp", async (req, res) => {
  if (req.query.code) {
    ...
    var authorization_token = Buffer.from(
      (await settings.getSettingData("docusignintegrationkey")) +
        ":" +
        (await settings.getSettingData("docusignsecretkey")),
      "utf8"
    ).toString("base64");
    ...
    axios({
      ...
      .then(async function (response) {
        ...
        if (row.length != 0) {
          await database.awaitQuery(
            `UPDATE access_token SET access_token = ?, refresh_token = ? WHERE type = "docusign"`,
            [oAuthToken.access_token, oAuthToken.refresh_token]
          );
        } else {
          await database.awaitQuery(
            `INSERT INTO access_token (access_token, refresh_token, type) VALUES (?, ?, "docusign")`,
            [oAuthToken.access_token, oAuthToken.refresh_token]
          );
        }
      })
  ...
  axios({
    ...
    .catch(function (error) {
      errorlogging.saveError(
        "error",
        "docusign",
        "Not possible to refresh the access token",
        error
      );
      console.log(date + " - " + error);
    });
...
app.post("/docusignwebhook", async (req, res) => {
  ...
  if (
    ...
    req.headers["x-docusign-signature-2"] ||
    req.headers["x-docusign-signature-3"] ||
    req.headers["x-docusign-signature-4"]
  ) {
    var hmac = crypto.createHmac(
      "sha256",
      await settings.getSettingData("docusignhmac")
    );
    ...
    if (
      gen_hash == req.headers["x-docusign-signature-1"] ||
      gen_hash == req.headers["x-docusign-signature-2"] ||
      gen_hash == req.headers["x-docusign-signature-3"] ||
      gen_hash == req.headers["x-docusign-signature-4"]
    ) {
      if (response.event) {
        ...
        var row = await database.awaitQuery(
          `SELECT * FROM docu_sign_events WHERE envelope_id = ? AND event = ?`,
          [response.data.envelopeId, response.event]
        );

        if (row.length == 0) {
          var PublicObjectSearchRequest = {
            filterGroups: [
              {
                filters: [
                  {
                    value: envelopeId,
                    propertyName: "deal_envelopeid",
                    operator: "EQ",
                  },
                ],
              },
          ...
          try {
            var apiResponse = await hubspotClient.crm.deals.searchApi.doSearch(
          ...
          await database.awaitQuery(
            `INSERT INTO docu_sign_events (envelope_id, event) VALUES (?, ?)`,
            [response.data.envelopeId, response.event]
          );
        }
            ...
            if (apiResponse.results[0]) {
              ...
              try {
                ...
                try {
                  ...
                  var docuSignUrl = replacePlaceholder(
                    await settings.getSettingData("docusigncontracturl"),
                    { envelopeId: envelopeId, dealId: dealId }
                  );
                  ...
                  try {
                    ...
                    if (
                      contactData.properties.aktuelle_liste != "Abgeschlossen"
                    ) {
                    ...
                    if (
                      changeValue ||
                      contactData.properties.export_software == "" ||
                      contactData.properties.export_software == "Nein" ||
                      contactData.properties.export_software == null ||
                      contactData.properties.export_software == "null"
                    ) {
                      properties["export_software"] = "Bereit";
                    }
                    ...
                    if (Object.keys(properties).length != 0) {
                      ...
                      console.log(
                        date +
                          " - Success: Docu Sign Webhook Accepted Contract - " +
                          envelopeId
                      );
                      errorlogging.saveError(
                        "success",
                ...
                errorlogging.saveError(
                  "error",
                  "hubspot",
                  "Error to load the Deal Data (" + deald + ")",
                  err
                );
                console.log(date + " - " + err);
              }
              ...
              try {
                ...
                try {
                  ...
                  try {
                    ...
                    var results = await envelopesApi.listRecipients(
                      accountId,
                      envelopeId
                    );
                    ...
                    if (
                      dealData.properties.docusign_absagegrund == "" ||
                      dealData.properties.docusign_absagegrund == null
                    ) {
                      properties["docusign_absagegrund"] = declinedReason;
                    }

                    if (
                      dealData.properties.docusign_absagedatum == "" ||
                      dealData.properties.docusign_absagedatum == null
                    ) {
                    ...
                    if (Object.keys(properties).length != 0) {
                      ...
                      errorlogging.saveError(
                        "success",
                        "docusign",
                        "Success: Docu Sign Webhook Canceled Contract",
                        envelopeId
                      );
                      console.log(
                        date + " - Success: Docu Sign Webhook Canceled Contract"
                      );
                    }
                    ...
                    if (
                      ...
                      if (dealData.properties["deal_gesellschaft"] == "DIS") {
                        var mailSubject = replacePlaceholder(
                          await settings.getSettingData("cancelmaildissubject"),
                          contactData.properties
                        );
                        var mailBody = replacePlaceholder(
                          await settings.getSettingData("cancelmaildistext"),
                          contactData.properties
                        );
                      } else if (
                        dealData.properties["deal_gesellschaft"] == "DIA"
                        ...
                        var mailSubject = replacePlaceholder(
                          await settings.getSettingData("cancelmaildiasubject"),
                          contactData.properties
                        );
                        var mailBody = replacePlaceholder(
                          await settings.getSettingData("cancelmaildiatext"),
                          contactData.properties
                        );
                      }
                      ...
                      await mailer.sendMail(
                        await settings.getSettingData("mailersentmail"),
                        contactData.properties.email,
                        mailSubject,
                        mailBody,
                        mailBody
      ...
      if (apiResponse.results[0]) {
        ...
        if (dealId == req.query["dealId"]) {
          ...
          try {
            ...
            envelopesApi
              .getDocument(accountId, envelopeId, "combined")
              .then(function (pdfBytes) {
                if (pdfBytes) {
                  try {
                    ...
                    return res.sendFile(
                      __dirname + "/public/showdocusigndocument/index.html"
                    );
                  }
                }
              })
...
cron.schedule("*/5 * * * *", async function () {
  ...
  var creds = JSON.parse(
    await settings.getSettingData("googlesheetprivatekey")
  );
  ...
  if (res.data) {
        ...
        if (importCol == "") {
          ...
          if (obj["Import"] == null && obj["E-Mail"] != null) {
            ...
            var properties = {
              ...
              hs_lead_status: "NEW",
              saltybrands__haben_sie_offene_forderungen_:
                obj["Haben Sie offene Forderungen?"],
              saltybrands__wie_viele_transaktionen_haben_sie_im_monat_:
                obj["Wie viele offene Forderungen haben Sie im Monat?"],
            };
            ...
            try {
              ...
              try {
                var response = (
                  await sheets.spreadsheets.values.update({
                    spreadsheetId: await settings.getSettingData(
                      "googlesheetid"
                    ),
              ...
              errorlogging.saveError(
                "success",
                "saltybrands",
                "Google Sheet SaltyBrands Import. (" + obj["E-Mail"] + ")",
                ""
              );
            } catch (err) {
              if (err.body["message"].includes("Contact already exists")) {
                ...
                errorlogging.saveError(
                  "error",
                  "saltybrands",
                  "Contact already exists. (" + obj["E-Mail"] + ")",
                  ""
                );
              } else {
                errorlogging.saveError(
                  "error",
                  "saltybrands",
                  "Not possibe to import. (" + obj["E-Mail"] + ")",
                  err.body["message"]
                );
              }
            }
    ...
    errorlogging.saveError(
      "error",
      "saltybrands",
      "Not possible to open google sheet ",
      ""
    );
  }
});
...
cron.schedule("0 * * * *", async function () {
  ...
  try {
    ...
    let envelopesInformation = await envelopesApi.listStatusChanges(accountId, {
      fromDate: date,
    });
    ...
    for (var i = 0; i < envelopesInformation.totalSetSize; i++) {
        ...
        filterGroups: [
          {
            filters: [
              {
                value: envelopesInformation.envelopes[i].envelopeId,
                propertyName: "deal_envelopeid",
                operator: "EQ",
              },
            ],
      ...
      if (typeof apiResponse.results[0] !== "undefined") {
        if (
          apiResponse.results[0].properties.docusign_envelope_status !=
          "envelope-" + envelopesInformation.envelopes[i].status
        ) {
        ...
        filters: [
          {
            ...
            propertyName: "docusign_envelope_status",
            operator: "EQ",
          },
          { propertyName: "deal_envelopeid", operator: "HAS_PROPERTY" },
          { propertyName: "docusign_url", operator: "NOT_HAS_PROPERTY" },
        ],
      },
    ],
    properties: ["docusign_url", "deal_envelopeid", "dealname"],
    limit: 100,
    after: 0,
  };
        ...
        try {
          ...
          try {
            ...
            for (var a = 0; a < docuSignFormData.length; a++) {
              docuSignFormatData[docuSignFormData[a]["name"]] =
                docuSignFormData[a]["value"];
            }
            ...
            if (Object.keys(properties).length != 0) {
              ...
              console.log(
                date +
                  " - Success: Docu Sign Checker Accepted Contract - " +
                  envelopeId
              );
              errorlogging.saveError(
                "success",
...
async function exportPDF() {
  ...
  var PublicObjectSearchRequest = {
    filterGroups: [
      {
        filters: [{ propertyName: "docusign_url", operator: "HAS_PROPERTY" }],
      },
    ],
    properties: ["docusign_url", "id"],
    limit: 100,
    after: 0,
  };
  ...
  for (var i = 0; i < total; i = i + 100) {
    ...
    for (var a = 0; a < result.length; a++) {
      ...
      console.log(
        counter,
        result[a].properties["hs_object_id"],
        result[a].properties["docusign_url"],
        result[a].properties["deal_envelopeid"],
        result[a].properties["docusign_contract_status"]
      );

      var envelopeId = result[a].properties["deal_envelopeid"];
      var PublicObjectSearchRequest = {
        filterGroups: [
          {
            filters: [
      ...
      if (result[a].properties["docusign_contract_status"] != null) {
        result[a].properties["docusign_contract_status"] = result[a].properties[
          "docusign_contract_status"
        ].replace(" ", "_");
      }
      ...
      if (
        !fs.existsSync(
          "./export_pdf/" + result[a].properties["docusign_contract_status"]
        )
      ) {
        fs.mkdirSync(
          "./export_pdf/" + result[a].properties["docusign_contract_status"]
      ...
      if (
        ...
        try {
          var test = await envelopesApi.getDocument(
            accountId,
            envelopeId,
            "combined"
          );
